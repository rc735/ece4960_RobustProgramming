...
void secantMethod(double *guess, double *a_final, double *Vds, double *Vgs,
                  double *Smeas, int paramSize, int measSize, int task)
{
  if(a_final == NULL || Vds == NULL || Vgs == NULL || Smeas == NULL)
    return;
  ...
  {
    a_prev[i] += da[i];
    dV_prev[i] = (generateV(a_prev, Vds, Vgs, Smeas, paramSize, measSize, task)
                    - generateV(a_final, Vds, Vgs, Smeas, paramSize, measSize, task))
                    / (da[i]);
    a_prev[i] = a_final[i] + da[i];
  }
  //cout << dV_prev[0] << " " << dV_prev[1] << endl << endl;;
...
//  double min, V_temp          // for linear search
  while(stop == 0)
  {
    ...
    for(int i = 0; i < paramSize; i++)
    {
      ...
      da[i] = a_final[i] * PERTURB_NUM;
      a_temp1[i] += da[i] ;
      dV_curr[i] = (generateV(a_temp1, Vds, Vgs, Smeas, paramSize, measSize, task)
                    - generateV(a_final, Vds, Vgs, Smeas, paramSize, measSize, task))
                    / (da[i]);
      a_temp1[i] = a_final[i];
    ...
    /**
    cout << "SLOPE:        " << dV_curr[0] << " " << dV_curr[1] 
                              << " " << dV_curr[2] << endl;
    cout << "PREV:         " << dV_prev[0] << " " << dV_prev[1]
                              << " " << dV_prev[2] << endl;
    cout << "DELT_A:       " << delt_a_final[0] << " " << delt_a_final[1] 
              << " " << delt_a_final[2] << endl;
    cout << "FINAL PARAMS: ";*/
