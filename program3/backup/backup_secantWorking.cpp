                  ...
                  double *Smeas, int paramSize, int measSize)
{
  ...
  // generating guess before initial guess
  for(int i = 0; i < paramSize; i++)
  {
    a_prev[i] += da[i];
  ...
  while(stop == 0)
  {
    ...
    cout << generateV(a_final, x, Smeas, paramSize, measSize) << " "
      ...
      {
        cout << (a_final[i] - a_prev[i]) << endl;
        delt_a[i] = dV_curr[i] * (a_final[i] - a_prev[i])
                                / (dV_curr[i] - dV_prev[i]);
      }
      else
      {
        cout << "asdfasdfasdfasdfasdfasdfasdfasdf" << endl;
        delt_a[i] = da[i];
      }
      //cout << dV_curr[i] << " " << (a_final[i] - a_prev[i]) << " ";
...
int main(void)
{
  ...
  double guess[SIZE_PARAM] = {log(11), -0.6};
